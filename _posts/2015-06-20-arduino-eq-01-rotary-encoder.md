...
It turns out that interpreting this signal on the arduino is actually kind of tricky. In fact, [Arduino Playground recommends some libraries](http://playground.arduino.cc/Main/RotaryEncoders) that work it all out for you. But what's the fun in letting someone else figure it out for you?

There are a few articles out there that offer do-it-yourself solutions, but of the two major methods I found, neither really suited my needs. First, this [Hobbytronics article](http://www.hobbytronics.co.uk/rotary-encoder-tutorial) suggests using a loop to check if the pins change: if you know that one of the pins changed from, say, high to low, you can check the other pin to know if you made a clockwise tick or a counterclockwise one (check out the dashed lines in the image above). I tried out the code from the article, but it didn't really work for me; the events weren't being registered correctly, and each click would usually record two or three extra clicks (in mixed directions).

The second method was actually pretty cool, from the [article on Arduino Playground](http://playground.arduino.cc/Main/RotaryEncoders). It suggests using interrupts -- they're special functions that get triggered when a signal on a pin changes (for my arduino, only two pins can signal an interrupt). The processor stops whatever it's doing, then executes the interrupt function, and goes back to what it was doing before. Nick Gammon has [a really cool post](http://gammon.com.au/interrupts) about how interrupts work on the arduino.
...
So I figured out a third method as an alternative to the first two. The gist is that the program waits for the full cycle of four events (and only those four events! any unexpected event restarts the loop), then records the cycle as either a clockwise tick or a counterclockwise one. Here's an outline of the loop:
...
void loop() {
  ...
  while (inputRegistered == 0) {   // loop until an input event is registered
    ...
    if (!(encoderA)) {  // first check if encoder A goes to LOW
      ...
      while (encoderB && !(encoderA)) {  // wait until B goes low...
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (encoderA) return;   // this is not the change you're looking for...
      }
      while (!(encoderA) && !(encoderB)) {  // now wait until A goes high
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (encoderB) return;
      }
      while (!(encoderB) && encoderA) {  // finally wait until B goes high
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (!(encoderA)) return;
      }
      ...
      while (encoderA && !(encoderB)) {  // wait until A goes low...
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (encoderB) return;
      }
      while (!(encoderB) && !(encoderA)) {  // now wait until B goes high
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (encoderA) return;
      }
      while (!(encoderA) && encoderB) {  // finally wait until A goes high
        readEncoderInWhile();
        if ( currentTime >= loopTime + loopLength ) return;
        if (!(encoderB)) return;
      }
